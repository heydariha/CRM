...
//        }        
        ...
        $paging_html .= '<div class="sg-toolbar pull-left">'.$obj->lang->line('tit_view').' '.($this->_page_row_start + 1).' '.$obj->lang->line('tit_to').' '.($this->_page_row_start + $this->_page_row_count).' '.$obj->lang->line('tit_of').' '.$this->_total_rows.'</div>';   
         
        $paging_html .= '<div class="btn-group pull-right" role="group">'; 
        $paging_html .= '<button type="'.$prev_button_type.'" name="'.$this->_config_grid_name.'page" value="1" class="fa fa-angle-double-right" title="'.$obj->lang->line('tit_go_to_first_page').'"></button>'; 
        $paging_html .= '<button type="'.$prev_button_type.'" name="'.$this->_config_grid_name.'page" value="'.$previous_page_number.'" class="fa fa-angle-right" title="'.$obj->lang->line('tit_go_to_previous_page').'"></button>'; 
        $paging_html .= '<button type="button" style="font-size:12px" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'.$obj->lang->line('tit_page').' '.$this->_page_number.' '.$obj->lang->line('tit_of').' '.$this->_total_page.'</button>';
        
        $paging_html .= '<button type="'.$next_button_type.'" name="'.$this->_config_grid_name.'page" value="'.$next_page_number.'" class="fa fa-angle-left" title="'.$obj->lang->line('tit_go_to_next_page').'"></button>'; 
        $paging_html .= '<button type="'.$next_button_type.'" name="'.$this->_config_grid_name.'page" value="'.$this->_total_page.'" class="fa fa-angle-double-left" title="'.$obj->lang->line('tit_go_to_last_page').'"></button>'; 
        $paging_html .= '</div>';
        $paging_html .= '</form>';
        return $paging_html;
    }
